...
class Twibot22_subnet_split(Dataset):
    def __init__(self, root='./data_preprocess/mentioned/', device='cpu', process=True, save=True):
        self.root = root
        self.device = device
        if process:
            ...
            df_support = pd.read_json('./support.json')
    ...
    def Des_embbeding(self):
        ...
        if not os.path.exists(path):
            ...
            des_tensor = torch.load(self.root + "des_tensor.pt", weights_only=True).to(self.device)
        print('Finished')
        return des_tensor
    ...
    def cat_prop_preprocess(self):
        ...
        if not os.path.exists(path):
            ...
            category_properties = torch.load(self.root + "cat_properties_tensor.pt", weights_only=True).to(self.device)
        print('Finished')
