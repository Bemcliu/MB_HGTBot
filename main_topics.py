...
(
    tweet_size, num_prop_size, comments_prop_size, embedding_dimension,
    dropout, num_heads, mlp_hidden_size, num_layers, temperature,
    lr, weight_decay
) = (
    768, 6, 6, 192, 0.3, 2, 192, 6, 0.3, 1e-3, 5e-3
)

current_time = time.strftime("%Y%m%d-%H%M%S")
log_dir = f'./log/MHCLBot/TrumpAttacked/{current_time}'
...
tweets_tensor, num_prop, comments_prop, edge_index, edge_type, labels, train_idx, val_idx, test_idx = dataset.dataloader()

saved_model_dir = 'saved_models/MHCLBot/TrumpAttacked'
...
def train(epoch):
    ...
    output, contrastive_loss = model(
        ...
        edge_index, edge_type, labels, len(train_idx)
    )
